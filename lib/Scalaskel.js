...
Scalaskel.prototype.decompose = /*_.memoize(*/function(groDessimal, factors) {
  var self = this;
  var result = [];
  if (groDessimal === 0) {
    return result;
  ...
  return result;
}/*,hashFunc).bind(Scalaskel.prototype)*/;


function merge(result, others) {
  //console.log('result: %s, others: %s',JSON.stringify(result),JSON.stringify(others));
