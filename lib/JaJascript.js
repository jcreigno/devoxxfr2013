...
JaJascript.prototype.optimize = function() {
  var self = this;
  var allPath = self.findAvaliablePaths([new State([], 0, this.nextInterleave(0))]);
  var max = {
    'gain': 0
  };
...
// donne le prochain vol pour lequel au moins une duree de vol est compris
JaJascript.prototype.nextInterleave = function(index) {
  var NUM_FLIGHTS = this.flights.length;
  var next = this.flights[index];
  var last = this.flights[NUM_FLIGHTS - 1];
  var minIntercalaire = next.DEPART + next.DUREE;
  for (var j = index; j < NUM_FLIGHTS && minIntercalaire < last.DEPART; j++) {
...
JaJascript.prototype.makeChildren = function(state) {
  ...
  for (var i = state.index; i < state.endIndex; i++) {
    ...
    if (nextIndex < NUM_FLIGHTS) {
      endIndex = this.nextInterleave(nextIndex);
    }
