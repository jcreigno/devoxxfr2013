...
function makeChildren(state) {
  ...
  for (var i = 0; i < state.unaffected.length; i++) {
    ...
    var filtered = state.unaffected.slice(i + 1);
    var children = [];
    for(var j = 0;j<filtered.length;j++){
    	 if(filtered[j].DEPART >= (flight.DEPART + flight.DUREE)){
        children.push(filtered[j]);
       }
    }
    results.push(new State(state.path.concat(flight), children, state.gain + flight.PRIX));
  }
  return results;
}
